...
func submit(w http.ResponseWriter, r *http.Request) {
	...
	out, err := os.Create(cwd+"/data/" + header.Filename)
	if err != nil {
		json.NewEncoder(w).Encode(err.Error())
		return
	}
	...
	json.NewEncoder(w).Encode("File '" + header.Filename + "' submited successfully")
}
...
<tr><td class="ntable"><a href="{{.}}">{{.}}</a></td><td class="t">&nbsp;</td><td class="dl"><a href="{{.}}?dl">Download</a></td></tr>
{{end}}
</tbody>
</table>
</div>
